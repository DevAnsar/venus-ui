...
const BarrowTableRow = ({ data }: { data: DataseedInterface }) => {
  return (
    <tr className="text-slate-50 hover:bg-[#ffffff14] cursor-pointer h-14">
      <td className="px-5 py-3 text-sm">
        <div className="flex items-center">
          ...
          <div className="ml-2">
            <p className=" whitespace-no-wrap">{data.underlyingSymbol}</p>
          </div>
        </div>
      </td>
      <td className="px-5 py-3 text-sm text-right">
        <p className="whitespace-no-wrap">
          {Number(+data.borrowVenusApy + data.borrowApy).toFixed(2)}%
        </p>
      </td>
      <td className="px-5 py-3 text-sm text-right">
      ...
      <td className="px-5 py-3 text-sm text-right">
        <p className="whitespace-no-wrap">
          ${nFormatter(Number(data.liquidity), 2)}
        </p>
      </td>
    </tr>
  );
...
export const BorrowTableMiniRow = ({ data }: { data: DataseedInterface }) => {
  return (
      ...
      <div className="flex items-center">
        <span className="px-5 py-3 text-sm  w-1/3">
          ...
          <p
            ...
            title={`${Number(+data.borrowVenusApy + +data.borrowApy).toFixed(
              2
            )}%`}
          >
            {Number(+data.borrowVenusApy + +data.borrowApy).toFixed(2)}%
